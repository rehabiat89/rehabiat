...
final class Storefront_Blog_Excerpt {
	...
	public function install() {
		...
		$tmp = get_template_directory();

		if( 'storefront' != basename( $tmp ) ) {
	...
	public function customize_register( $wp_customize ) {
		...
		$wp_customize->add_setting(
			...
			array(
				'default' => __( 'Read more', 'woa-sf-blog-excerpt' ),
			)
		);
		$wp_customize->add_setting(
	...
	public function post_excerpt() {
		...
		$content = wp_trim_words( $content, get_theme_mod( 'woa_sf_blog_excerpt_word_count', 55 ), get_theme_mod( 'woa_sf_blog_excerpt_end', '&hellip;' ) );

		echo apply_filters( 'the_excerpt', $content );
		?>
		<p class="read-more"><a class="button" href="<?php the_permalink(); ?>"><?php echo get_theme_mod( 'woa_sf_blog_excerpt_button_text', __( 'Read more', 'woa-sf-blog-excerpt' ) ); ?></a></p>
		</div><!-- .entry-content -->
		<?php
	}
